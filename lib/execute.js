...
const execute = _.promise.make((self, done) => {
    ...
    self.postgres.query(self.statement, self.params)
        .then(result => {
            ...
            self.rows = last_result.rows.map(d => Object.assign({}, d));
            self.count = last_result.rowCount;
            self.row = self.rows.length ? self.rows[0] : null;
...
const execute_p = (statement, params) => _.promise.make((self, done) => {
    _.promise.make(self)
        ...
        .then(execute)
        .then(_.promise.done(done, self, "postgres_result,rows,row,count"))
        .catch(done)
})
