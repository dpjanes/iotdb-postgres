...
const initialize = _.promise.make((self, done) => {
    ...
    assert.ok(self.postgresd.url, `${method}: expected self.postgresd.url`)
    ...
    if (self.postgresd.url) {
        self.postgres.client = new postgres.Client(self.postgresd.url)
        self.postgres.client.connect(error => {
            if (error) {
                return done(error);
            }

            self.postgres.clients[self.postgresd.url] = self.postgres.client
